...
import SmartCaptureCamera from '@/components/capture/SmartCaptureCamera';
import SelfLearningDashboard from '@/components/dashboard/SelfLearningDashboard';
import { getApiBaseUrl } from '@/lib/api';
import { UserSession } from '@/components/shared/LoginPage';
...
export default function OfficerDashboard({ session, onSelectInspection }: OfficerDashboardProps) {
  ...
  const passCount = safeInspections.filter(i => i.overall_status === 'PASS').length;
  const failCount = safeInspections.filter(i => i.overall_status === 'FAIL' || i.overall_status === 'NON_COMPLIANT' || i.overall_status === 'NEEDS_HUMAN_REVIEW').length;
  const totalCount = safeInspections.length;
